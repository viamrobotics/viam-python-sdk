...
class RobotService(RobotServiceBase, ResourceRPCServiceBase):
    def _generate_metadata(self) -> List[ResourceName]:
        ...
        for resource in self.manager.resources.values():
            ...
            if (
                resource.SUBTYPE == Sensor.SUBTYPE
                and MovementSensor.get_resource_name(resource.name)
                in self.manager.resources
            ):
                continue
    ...
    async def _generate_status(
        self, resource_names: Iterable[ResourceName]
    ) -> List[Status]:
        ...
        for resource in self.manager.resources.values():
            for registration in Registry.REGISTERED_SUBTYPES().values():
                if isinstance(resource, registration.resource_type):
                    if (
                        resource_names
                        and resource.get_resource_name(resource.name)
    ...
    async def ResourceNames(
        self, stream: Stream[ResourceNamesRequest, ResourceNamesResponse]
    ) -> None:
    ...
    async def GetStatus(
        self, stream: Stream[GetStatusRequest, GetStatusResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
    ...
    async def StreamStatus(
        self, stream: Stream[StreamStatusRequest, StreamStatusResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        interval = 1
    ...
    async def GetSessions(
        self, stream: Stream[GetSessionsRequest, GetSessionsResponse]
    ) -> None:
        raise MethodNotImplementedError("GetSessions").grpc_error

    async def StartSession(
        self, stream: Stream[StartSessionRequest, StartSessionResponse]
    ) -> None:
        raise MethodNotImplementedError("StartSession").grpc_error

    async def SendSessionHeartbeat(
        self, stream: Stream[SendSessionHeartbeatRequest, SendSessionHeartbeatResponse]
    ) -> None:
        raise MethodNotImplementedError("SendSessionHeartbeat").grpc_error

    async def TransformPCD(
        self, stream: Stream[TransformPCDRequest, TransformPCDResponse]
    ) -> None:
    ...
    async def GetCloudMetadata(
        self, stream: Stream[GetCloudMetadataRequest, GetCloudMetadataResponse]
    ) -> None:
