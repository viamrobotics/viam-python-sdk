...
class SLAMServiceClient(ServiceClientBase, ReconfigurableResourceRPCClientBase):
    ...
    async def get_point_cloud_map(self, name: str, *, timeout: Optional[float] = None) -> GetPointCloudMapResponse:
        request = GetPointCloudMapRequest(name=name)
        response: GetPointCloudMapResponse = await self.client.GetPointCloudMap(request, timeout=timeout)
        return response

    async def get_internal_state(self, name: str, *, timeout: Optional[float] = None) -> GetInternalStateResponse:
        """_summary_
        ...
        Returns:
            GetInternalStateResponse: Chunk of the internal state of the SLAM algorithm required to continue mapping/localization

        """
        request = GetInternalStateRequest(name=name)
        response: GetInternalStateResponse = await self.client.GetInternalState(request, timeout=timeout)
        return response
