...
def flat_tensors_to_ndarrays(flat_tensors: FlatTensors) -> Dict[str, NDArray]:
    ...
    def make_ndarray(flat_data, dtype, shape):
        ...
        make_array = (
            np.frombuffer if dtype == np.int8 or dtype == np.uint8 else np.array
        )
    ...
    for name, flat_tensor in flat_tensors.tensors.items():
        property_name = flat_tensor.WhichOneof("tensor") or flat_tensor.WhichOneof(
            b"tensor"
        )
        if property_name:
            tensor_data = getattr(flat_tensor, property_name)
            flat_data, dtype, shape = (
                tensor_data.data,
                property_name_to_dtype[property_name],
                flat_tensor.shape,
            )
...
def ndarrays_to_flat_tensors(ndarrays: Dict[str, NDArray]) -> FlatTensors:
    ...
    def get_tensor_data(ndarray: NDArray):
        ...
        if (
            tensor_data_class == FlatTensorDataInt8
            or tensor_data_class == FlatTensorDataUInt8
        ):
            data = (
                data.tobytes()
            )  # as per the proto, int8 and uint8 are stored as bytes
        elif (
            tensor_data_class == FlatTensorDataInt16
            or tensor_data_class == FlatTensorDataUInt16
        ):
            data = data.astype(
                np.uint32
            )  # as per the proto, int16 and uint16 are stored as uint32
    ...
    for name, ndarray in ndarrays.items():
        ...
        tensors_mapping[name] = FlatTensor(
            shape=ndarray.shape, **{prop_name: prop_value}
        )
