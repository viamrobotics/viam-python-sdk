...
class MovementSensorRPCService(
    MovementSensorServiceBase, ResourceRPCServiceBase[MovementSensor]
):
    """
    ...
    async def GetLinearVelocity(
        self, stream: Stream[GetLinearVelocityRequest, GetLinearVelocityResponse]
    ) -> None:
    ...
    async def GetAngularVelocity(
        self, stream: Stream[GetAngularVelocityRequest, GetAngularVelocityResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
    ...
    async def GetLinearAcceleration(
        self,
        stream: Stream[GetLinearAccelerationRequest, GetLinearAccelerationResponse],
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
        sensor = self.get_resource(name)
        timeout = stream.deadline.time_remaining() if stream.deadline else None
        acceleration = await sensor.get_linear_acceleration(
            extra=struct_to_dict(request.extra),
            timeout=timeout,
            metadata=stream.metadata,
        )
    ...
    async def GetCompassHeading(
        self, stream: Stream[GetCompassHeadingRequest, GetCompassHeadingResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
    ...
    async def GetOrientation(
        self, stream: Stream[GetOrientationRequest, GetOrientationResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
    ...
    async def GetAccuracy(
        self, stream: Stream[GetAccuracyRequest, GetAccuracyResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
    ...
    async def GetReadings(
        self, stream: Stream[GetReadingsRequest, GetReadingsResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
        ...
        response = GetReadingsResponse(
            readings=sensor_readings_native_to_value(readings)
        )
        await stream.send_message(response)
