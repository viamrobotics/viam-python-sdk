...
class PowerSensorRPCService(
    PowerSensorServiceBase, ResourceRPCServiceBase[PowerSensor]
):
    """
    ...
    async def GetVoltage(
        self, stream: Stream[GetVoltageRequest, GetVoltageResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
    ...
    async def GetCurrent(
        self, stream: Stream[GetCurrentRequest, GetCurrentResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
