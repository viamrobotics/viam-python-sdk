...
class GantryRPCService(UnimplementedGantryServiceBase, ResourceRPCServiceBase[Gantry]):
    ...
    async def GetKinematics(self, stream: Stream[GetKinematicsRequest, GetKinematicsResponse]) -> None:
        ...
        gantry = self.get_resource(request.name)
        timeout = stream.deadline.time_remaining() if stream.deadline else None
        format, data, meshes_by_urdf_filepath = await gantry.get_kinematics(
            extra=struct_to_dict(request.extra), timeout=timeout, metadata=stream.metadata
        )
        response = GetKinematicsResponse(format=format, kinematics_data=data, meshes_by_urdf_filepath=meshes_by_urdf_filepath)
        await stream.send_message(response)
