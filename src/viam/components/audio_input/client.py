...
class AudioInputClient(AudioInput, ReconfigurableResourceRPCClientBase):
    ...
    async def stream(
        self, *, timeout: Optional[float] = None, **__
    ) -> MediaStream[Audio]:
        async def read() -> AsyncIterator[Audio]:
            async with self.client.Chunks.open(timeout=timeout) as chunks_stream:
                ...
                response: Union[
                    ChunksResponse, None
                ] = await chunks_stream.recv_message()
                if not response:
                    await (
                        chunks_stream.recv_trailing_metadata()
    ...
    async def get_properties(
        ...
        response: PropertiesResponse = await self.client.Properties(
