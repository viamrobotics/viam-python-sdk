...
class CameraRPCService(CameraServiceBase, ResourceRPCServiceBase[Camera]):
    ...
    async def GetImages(
        self, stream: Stream[GetImagesRequest, GetImagesResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
        ...
        for img in images:
            try:
                ...
                img_bytes = img.data
                img_bytes_lst.append(
                    Image(source_name=name, format=fmt, image=img_bytes)
                )
            finally:
                img.close()
    ...
    async def RenderFrame(self, stream: Stream[RenderFrameRequest, HttpBody]) -> None:  # pyright: ignore [reportInvalidTypeForm]
        ...
        timeout = stream.deadline.time_remaining() if stream.deadline else None
        image = await camera.get_image(
            mimetype, timeout=timeout, metadata=stream.metadata
        )
        try:
        ...
        response = HttpBody(
            data=img,
            content_type=image.mime_type if isinstance(image, RawImage) else mimetype,
        )  # type: ignore
        await stream.send_message(response)

    async def GetPointCloud(
        self, stream: Stream[GetPointCloudRequest, GetPointCloudResponse]
    ) -> None:
        request = await stream.recv_message()
        assert request is not None
        name = request.name
