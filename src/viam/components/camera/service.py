...
class CameraRPCService(CameraServiceBase, ResourceRPCServiceBase[Camera]):
    ...
    async def GetImages(self, stream: Stream[GetImagesRequest, GetImagesResponse]) -> None:
        ...
        timeout = stream.deadline.time_remaining() if stream.deadline else None
        images, metadata = await camera.get_images(filter_source_names=request.filter_source_names, timeout=timeout, metadata=stream.metadata)
        img_bytes_lst = []
        for img in images:
            fmt = img.mime_type.to_proto()
            img_bytes = img.data
            img_bytes_lst.append(Image(source_name=name, format=fmt, image=img_bytes, mime_type=img.mime_type.value))
