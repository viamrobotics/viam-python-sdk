...
class PoseTrackerRPCService(
    PoseTrackerServiceBase, ResourceRPCServiceBase[PoseTracker]
):
    """
    ...
    async def DoCommand(
        ...
        timeout = stream.deadline.time_remaining() if stream.deadline else None
        result = await pose_tracker.do_command(
    ...
    async def GetGeometries(
        ...
        pose_tracker = self.get_resource(request.name)
        timeout = stream.deadline.time_remaining() if stream.deadline else None
        geometries = await pose_tracker.get_geometries(
