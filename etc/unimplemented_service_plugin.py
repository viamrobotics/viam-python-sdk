...
from typing import List, Any, Collection, Iterator, NamedTuple
...
def render(
    ...
    for service in services:
        package_name = _proto2grpc_module_name(proto_file)
        service_base_name = f"{service.name}Base"
        buf.add(f"from .{package_name.split('.')[-1]} import {service_base_name} as _{service_base_name}")

        buf.add("")
        buf.add("")
        buf.add("class Unimplemented{}(_{}):", service_base_name, service_base_name)
        with buf.indent():
...
def _proto2unimplemented_module_name(proto_file_path: str) -> str:
    return _base_module_name(proto_file_path) + "_unimplemented_grpc"
...
def main() -> None:
    ...
    for file_to_generate in request.file_to_generate:
        ...
        if services:
            ...
            module_name = _proto2unimplemented_module_name(file_to_generate)
