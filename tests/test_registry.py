...
def test_components_register_themselves_correctly():
    ...
    assert 'gps' in Registry.REGISTERED_COMPONENTS
    assert 'gripper' in Registry.REGISTERED_COMPONENTS
    assert 'imu' in Registry.REGISTERED_COMPONENTS
...
def test_registration():
    ...
    Registry.register(
        ComponentRegistration(
            FakeComponent,
            'fake_component',
            FakeComponentService,
            lambda name, channel: FakeComponentClient(name, channel)
        )
    )
